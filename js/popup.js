...
class api {
  constructor(opts = {}, cb) {
    ...
    //该实例对象
    let slotDom = (this.Popup = `<div class="common_popup${version} common_popup v_hide ${
    ...
    //该实例插槽
    let $slotContent = (this.$PopupContent = $slotDom.find(
      "._common-popup-content"
    ));
  ...
  show(strDom) {
    ...
    this.autoInitContent &&
      this.constructor._setPopupContent.call(this, strDom);
  ...
  static _getTriggerOffset() {
    ...
    if (!this.$trigger.length) {
      throw new Error(
        `【${this._trigger}】is undefined, can\`t find this jquery elem`
      );
    }
  ...
  static _setTriggerPosition(reset) {
    ...
    this.trigger_position = {
      //[x,y]
      top: [
        this.trigger_info.left,
        this.trigger_info.top - _height - POINTER_HEIGHT
      ],
      left: [
        this.trigger_info.left - _width - POINTER_HEIGHT,
        this.trigger_info.top
      ],
      right: [
        this.trigger_info.left + this.trigger_info.width + POINTER_HEIGHT,
        this.trigger_info.top
      ],
      bottom: [
        this.trigger_info.left,
        this.trigger_info.top + this.trigger_info.height + POINTER_HEIGHT
      ]
    };
  ...
  static _setPopupStyle() {
    let objClass = {
      width:
        this.constructor._getNum.call(this, this._width) +
        this.constructor._getUnit.call(this, this._width),
      height:
        this.constructor._getNum.call(this, this._height) +
        this.constructor._getUnit.call(this, this._height)
    };
    ...
    let width_ed = +this.$Popup
      .find(`._common-popup${this.version}`)
      .css("width")
      .replace(/[^0-9]/gi, "");
    let isOverflow =
      width_ed + this.$trigger.offset().left + this.position[0] >
      $window.width();
    ...
    pointerStyle &&
      this.$Popup
        .find(`._common-popup${this.version} .pointer`)
        .css(pointerStyle);
  }
  ...
  static eventTrigger() {
    ...
    if (that.triggerType === "hover") {
      $(document)
        .on("mouseenter", that["_trigger"], function() {
